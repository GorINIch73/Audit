...
// дата (?<=\D|^)(?<day>[0-3][1-9])(?<sep>[^\w\s])(?<month>1[0-2]|0[1-9])\k<sep>(?<year>\d{2}|\d{4})(?=\D|$)
     ui -> lineEdit_regex -> setText ( "(?<name>контракту|контракт|контр|конт|кон|договору|договор|догов|дог)([.]|)\\s{0,}([N№]|)\\s{0,}(?<number>\\S{1,})\\s{0,}(от|)\\s{0,}(?<=\\D|^)(?<day>[0-2][0-9]|[3][0-1])(?<sep>[^\\w\\s])(?<month>1[0-2]|0[1-9])\\k<sep>(?<year>\\d{4}|\\d{2})(?=\\D|$)" );
